...
export async function POST(req:Request){
 ...
 try{const bytes=await limitedBody(req,2700000);const form=await new Response(bytes,{headers:{'Content-Type':req.headers.get('content-type')||''}}).formData();const file=form.get('audio');if(!(file instanceof File)||file.size>2500000||!/^audio\/(webm|ogg|mp4|wav|mpeg)/.test(file.type))return Response.json({error:'Use a short audio recording.'},{status:400});
