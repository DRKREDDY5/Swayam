...
const Schema=z.union([z.object({greeting:z.literal(true),language:z.enum(['te','en'])}).strict(),z.object({guideId:z.string().max(30),step:z.number().int().min(0).max(20),language:z.enum(['te','en'])}).strict(),z.object({speechToken:z.string().max(20000)}).strict()]);
export async function POST(req:Request){
 ...
 else if('greeting' in p){text=GREETING[p.language];language=p.language;}
 ...
 const r=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(e.ELEVENLABS_VOICE_ID)}`,{method:'POST',headers:{'xi-api-key':e.ELEVENLABS_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({text,model_id:'eleven_v3',language_code:language}),signal});
 if(!r.ok||!r.headers.get('Content-Type')?.startsWith('audio/'))throw Error();
 // Validate a bounded, nonempty body before reporting audio success. This
 // affects only optional audio; written answers have already returned.
 const audio=await limitedBody(r,8_000_000);signal.throwIfAborted();if(!audio.byteLength)throw Error();
