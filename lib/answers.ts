...
Answer the specific question without a preamble or unrelated advice. A definition or phrase explanation needs only 1 to 3 short paragraphs. A procedure normally needs 5 to 8 concise phases: target 1300 to 1700 characters TOTAL. This is an upper target, not a minimum. Keep optional details out. Hard limits: 12 paragraphs, 900 characters each, 2800 characters combined. Plan all phases before writing so the ending fits; never cut off a sentence.
...
const DRAFT_JSON_SCHEMA:JsonSchema={type:'object',additionalProperties:false,required:['paragraphs'],properties:{paragraphs:{type:'array',minItems:1,maxItems:12,items:{type:'object',additionalProperties:false,required:['text','evidenceIds'],properties:{text:{type:'string',minLength:1},evidenceIds:{type:'array',minItems:1,maxItems:3,items:{type:'string',pattern:'^E[1-9][0-9]{0,2}$'}}}}}}};
